...
function predict_impact {
  ...
  local sma is orbit_:semimajoraxis.
  local true_anomaly is orbit_:trueanomaly.
  
  // Calculate true anomaly at impact using orbital equation
  // r = a(1-e^2)/(1 + e*cos(θ))
  // Solve for θ where r = body_radius + height
  local p is sma * (1 - ecc^2). // semi-latus rectum
  local cos_theta is (p/body_radius - 1) / ecc.
  
  // Get both solutions for true anomaly
  local theta1 is arccos(cos_theta).
  local theta2 is 360 - theta1.
  
  // Choose the solution that's ahead of us in the orbit
  local impact_true_anomaly is 0.
  if (theta1 - true_anomaly > 0) and (theta1 - true_anomaly < 180) {
    set impact_true_anomaly to theta1.
  } else {
    set impact_true_anomaly to theta2.
  }
