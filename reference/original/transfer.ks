...
// === TRANSFER PLANNER ===
//
// Plan a rendezvous with the current target from a (near-)circular orbit,
...
// intercept (plan the match burn at closest approach).
...
if in_bound:length = 0 {
  ...
  print "Next: run polish. run next. run intercept.".
}
